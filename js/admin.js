...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedPassword) {
    ...
    loadAllData();
    document.getElementById('admin-nav').innerHTML += '<li><a href="https://yousef-r-younis.github.io/club-members/index.html">View Members</a></li>';
  }
});
...
function displayApplicationsAdmin(applications) {
  ...
  applications.forEach(app => {
    ...
    if (app.status === 'pending') {
      actions = `
      <span style="display: flex; flex-direction: column; gap: 0.5rem; align-items: stretch;">
        <button onclick="updateStatus(${app.rowIndex}, 'approved')" class="btn-primary" style="padding: 0.4rem 0.8rem; font-size: 0.9rem; background: #2ecc71;">Approve</button>
        <button onclick="updateStatus(${app.rowIndex}, 'rejected')" class="btn-secondary" style="padding: 0.4rem 0.8rem; font-size: 0.9rem; background: #e74c3c;">Reject</button>
      </span>
      `;
    } else {
